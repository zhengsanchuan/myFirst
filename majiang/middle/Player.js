...
Player.prototype = {
    ...
    getJoinedRoom: function() {
        return this.user.marks.roomId;
    },
    ...
    // 保存回放数据
    savePlayBackDate: function(playBackDate) {
        var playBackDates = this.user.playBackDates || [];
        playBackDates.push(playBackDate);
        if (playBackDates.length > 30) {
            playBackDates = playBackDates.splice(0, playBackDates.length - 30);
        }
        this.user.playBackDates = playBackDates;
        this.markDirty();
    },
     ...
     * @returns {Array}
     */
    getPlayBackDate: function (roomId, curRound) {
        var flag = false;
        var date = null;
        this.user.reports.forEach(function (report) {
            if (flag || !report) {
                return;
            }
            report.playBackDate.forEach(function (playBackDate) {
                if (flag) {
                    return;
                }
                if (playBackDate.roomId == roomId) {
                    flag = true;
                }
            });
            if (flag) {
                date = report.playBackDate;
            }
        });
        var val = null;
        flag = false;
        date.forEach(function (eumDate) {
            if (flag) {
                return;
            }
            if (eumDate && eumDate.curRound == curRound) {
                flag = true;
                val = eumDate;
            }
        });
     ...
     * @returns {Array}
     */
    getReportsDetail: function() {
        var reportdate = [];
        this.user.reports.forEach(function (date) {
            reportdate.push(date.reportsDate);
        });
        return reportdate;
    },
     ...
     * @returns {Array}
     */
    getItenReportsOneDate: function(roomId) {
        var flag = false;
        var reportdate = null;
        this.user.reports.forEach(function (date) {
            if(flag){
            ...
            if(date.reportsDate.roomID == roomId){
                flag = true;
                reportdate = date.playBackDate;
            }
        });

        var roomInfo = [];
        flag = false;
        reportdate.forEach(function(date){
            if(flag){
                return;
            }
            if(date && date.playBattle && date.roomId == roomId){
                roomInfo.push(date.playBattle);
            }
        });
        return roomInfo;
    },
    ...
    decCards: function(cards) {
        ...
        ERROR(util.format("decCards, MOD-%d, CUR-%d"
