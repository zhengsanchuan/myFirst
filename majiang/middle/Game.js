...
exports.joinRoom = function(wsConn, rState, reqArgs, player){
    ...
    do {
        ...
        // if (player.getJoinedRoom() == roomId || player.hasJoinedRoom() ) {
        //     break;
        // }
 ...
 * @param player
 */
exports.matchGame = function(wsConn, rState, reqArgs, player){
    var gameType = +reqArgs.gameType;
    ...
    if (!gameServ) {
        wsConn.sendMsg({
            code    : ProtoID.SMSG_START_MATCH,
            state   : ProtoState.STATE_GAME_NOT_SUPPORT
        });
        ERROR(util.format("Player %d request matchGame %d", player.uid, gameType));
        return;
    }
    ...
    allocRoomId(function(err, roomId){
        if (err) {
            ...
            gameServ.sendMsg({
                code    : ProtoID.SMSG_REQ_START_MATCH,
                args    : reqArgs
            });
        }
    });
...
// 游戏匹配成功 - 游戏服务器响应
exports.onRoomMatched = function(wsConn, rState, reqArgs){
    var uid = +reqArgs.matchUid;
    var roomId = +reqArgs.roomId;
    var rRoomId = +reqArgs.rRoomId;
    ...
    PlayerMgr.getPlayer(uid, function(player){
        if (player) {
            if (rState == ProtoState.STATE_OK) {
                if (roomId != rRoomId) {
                    // 这个房间号没有使用，释放
                    GlobalInfo.freeRoomId(rRoomId);
                }

                GSMgr.addRoomId2Sid(roomId, serv.getSid());
                player.getConn().sendMsg({
                    code    : ProtoID.SMSG_START_MATCH,
                    state   : ProtoState.STATE_OK,
                    args    : {
                ...
                DEBUG(util.format("Player %d match game %d", player.uid, roomId));
            } else {
                player.getConn().sendMsg({
                    code    : ProtoID.SMSG_START_MATCH,
                    state   : ProtoState.STATE_GAME_CREATE_ROOM_FAILED,
