...
// 匹配游戏
exports.matchGame = function(wsConn, rState, reqArgs){
    var matchUid = reqArgs.uid;              // 房间创建者
    var gameType = reqArgs.gameType;    // 游戏类型
    var roomId = reqArgs.roomId;        // 房间号

    if (Config.GameType != gameType) {
        wsConn.sendMsg({
            code    : ProtoID.SMSG_RESP_START_MATCH,
            state   : ProtoState.STATE_FAILED,
            args    : {
                matchUid    : matchUid,         // 玩家UID
                rRoomId     : roomId,           // 分配的房间号
            }
        });
        ERROR(util.format("Middle request matchGame %d game, but not support", gameType));
        return;
    }

    var SubGameLogic = GameMgr.getSubGame();
    var newRoom = SubGameLogic.matchRoom(matchUid, roomId, reqArgs);
    if (newRoom) {
        wsConn.sendMsg({
            code    : ProtoID.SMSG_RESP_START_MATCH,
            state   : ProtoState.STATE_OK,
        ...
        ERROR(util.format("Start match failed, %d - %d", matchUid, gameType));
    }
};