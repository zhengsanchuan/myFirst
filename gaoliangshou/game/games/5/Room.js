...
Room.prototype = {
     ...
     * @param card
     */
    onPlayerReqEatCard: function (uid, cards) {
        var playerIndex = this.getPlayerIndex(uid);
        var player = this.players[playerIndex];
        ...
        cards.push(eatMj);
        if (!player.handCards.eatAble(cards)) {
            DEBUG(util.format("玩家[%s]碰牌[%d]失败,未能满足吃牌条件", uid, eatMj));
            return;
        }
        this.preEatMj = [];
        cards.forEach(function (card) {
            this.preEatMj.push(card);
        });

        this.doEat(playerIndex, cards);
    },
    ...
    doEat: function (playerIndex, eatMj) {
        var player = this.players[playerIndex];
    ...
    doPlay: function (playerIndex, card) {
        ...
        this.enumPlayers(function (ePlayerIndex, ePlayer) {
            if (ePlayerIndex != playerIndex && !ePlayer.over) {
                ...
                var huAble = Enum.validContains(this.playMeThod, Enum.PlayMeThod.ONLY_ZM) ? false : ePlayer.handCards.huAble(card);
     ...
     * @param type
     */
    doGang: function (playerIndex, gangMj, gangType, force) {
        var player = this.players[playerIndex];
        var cards = player.handCards;
        ...
        player.eatState = null;
        player.huState = null;
        if (gangType == Enum.GangType.MIAN_XIA_GANG && !force) {
            var qiangGangHu = false;
            for (var index in this.players) {
                var p = this.players[index];
                if (p.index != player.index && !p.over && p.uid > 0) {
                    var huAble = Enum.validContains(this.playMeThod, Enum.PlayMeThod.ONLY_ZM) ? false : p.handCards.huAble(gangMj); //todo 后面可能需要增加不能胡条件 (倍数不够)
        ...
        if (gangType == Enum.GangType.AN_GANG || (!this.hued(playerIndex) && !this.huIng(playerIndex)) || force) {
            ...
            this.billGang(playerIndex, gangType);
    ...
    calcHuData: function (playerIndex, cards, huType, huPaiType, isZm) {
        ...
        //牌的基础类型翻数
        // var huTypeMuti = Enum.PaiTypeMuti[huPaiType];
        // var huPaiTypeMuti = Enum.PaiTypeMuti[huPaiType];
        // if (cards.isQingYiSe()) {
        //     detail.push(Enum.PaiTypeMuti.QING_YI_SE);
        //     totalMultiple *= Enum.PaiTypeMuti[Enum.PaiTypeMuti.QING_YI_SE];
        // }
        // if (cards.isJingDiaoDiao()) {
        //     detail.push(Enum.PaiTypeMuti.JING_DIAO_DIAO);
        //     totalMultiple *= Enum.PaiTypeMuti[Enum.PaiTypeMuti.JING_DIAO_DIAO];
        // }
        // if (cards.isDaiYao()) {
        //     detail.push(Enum.PaiTypeMuti.DAI_YAO);
        //     totalMultiple *= Enum.PaiTypeMuti[Enum.PaiTypeMuti.DAI_YAO];
        // }
        // if (cards.isJiangDui()) {
        //     detail.push(Enum.PaiTypeMuti.JIANG_DUI);
        //     totalMultiple *= Enum.PaiTypeMuti[Enum.PaiTypeMuti.JIANG_DUI];
        // }
        // if (cards.isJueZhang()) {
        //     detail.push(Enum.PaiTypeMuti.JUE_ZHANG);
        //     totalMultiple *= Enum.PaiTypeMuti[Enum.PaiTypeMuti.JUE_ZHANG];
        // }
        // if (cards.isZmJiaDi()) {
        //     detail.push(Enum.PaiTypeMuti.ZI_MO_JIA_DI);
        //     totalMultiple *= Enum.PaiTypeMuti[Enum.PaiTypeMuti.ZI_MO_JIA_DI];
        // }
    ...
    billHu: function (winPlayerIndex, totalMultiple, isZm) {
        ...
        //没有胡牌的人返杠钱
        for (var pIndex in this.players) {
            if (this.players[pIndex].index == winPlayerIndex) {
                continue;
            }
            var player = this.players[pIndex];
            for (var index in player.gangDtail) {
                this.players[index].gangScore += player.gangDtail[index];
                player.gangScore -= player.gangDtail[index];
            }
        }
    },

    //结算杠
    billGang: function (winPlayerIndex, gangType) {
        ...
        switch (gangType) {
            ...
            case Enum.GangType.MIAN_XIA_GANG:
                ...
                break;
            case Enum.GangType.ZHI_GANG:
                this.players[this.prePlayer].gangScore -= muti;
                player.gangScore += muti;
                if (!player.gangDtail.hasOwnProperty(this.prePlayer)) {
                    player.gangDtail[this.prePlayer] = muti;
                } else {
                    player.gangDtail[this.prePlayer] += muti;
                }
                // for (var index in this.players) {
                ...
                //     if (p.index != player.index && p.uid > 0 && !p.over) {
                //         // losePlayer.push(p.index);
                //         if (p.index == this.prePlayer) {
                //             p.gangScore -= muti * 3;
                //             player.gangScore += muti * 3;
                //         } else {
                //             p.gangScore -= muti;
                //             player.gangScore += muti;
                //         }
            ...
            if (!this.hued(null)) {
                for (var index in this.players) {
                    ...
                    if (p.pengState == Enum.GrabState.GRABED) {
                        ...
                        this.doGang(p.index, card, p.handCards.gangType(card), true);
                    }
                    else if (p.eatState == Enum.GrabState.GRABED) {
                        this.doEat(p.index, this.preEatMj);
                    }
                }
            }
            this.state = Enum.GameState.PLAY;
